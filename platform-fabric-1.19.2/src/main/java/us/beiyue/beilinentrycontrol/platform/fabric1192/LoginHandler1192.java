...
public final class LoginHandler1192 {
	...
	private static boolean forceDisconnect(Object handler, String message) {
		...
		BeilinEntryControl1192.LOGGER.error("forceDisconnect: no working disconnect for {}", handler.getClass().getName());
		...
		while (c != null) {
			for (Field f : c.getDeclaredFields()) {
				...
				try {
					...
					BeilinEntryControl1192.LOGGER.debug("Profile field get failed", e);
				}
			}
			c = c.getSuperclass();
		}
