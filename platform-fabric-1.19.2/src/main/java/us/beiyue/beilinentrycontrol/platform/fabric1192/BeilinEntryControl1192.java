...
public final class BeilinEntryControl1192 implements ModInitializer {
	public static final String MOD_ID = "beilin-entry-control";
	public static final Logger LOGGER = LoggerFactory.getLogger(MOD_ID);

	private static ModConfig1192 config;
	...
	public void onInitialize() {
		...
		try {
			config = ModConfig1192.loadOrCreate();
		} catch (Exception e) {
			throw new IllegalStateException(
				"Failed to load Beilin Entry Control config from " + ModConfig1192.configPath(),
				e
			);
		}
		if (!config.isApiKeyConfigured()) {
			throw new IllegalStateException(
				"Beilin Entry Control requires an apiKey. Please configure " + ModConfig1192.configPath()
			);
		}
		...
		LoginHandler1192.setApiClient(apiClient);
		LoginHandler1192.setGateState(gateState);
		LoginHandler1192.register();
	...
	private void onServerStarting(MinecraftServer server) {
		...
		PlatformHooks1192 hooks = new PlatformHooks1192(server);
