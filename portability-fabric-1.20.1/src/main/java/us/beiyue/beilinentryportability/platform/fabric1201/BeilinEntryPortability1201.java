...
public final class BeilinEntryPortability1201 implements ModInitializer {
	...
	public void onInitialize() {
		...
		try {
			addonConfig = PortabilityAddonConfig1201.loadOrCreate();
			controlConfig = SharedControlConfig1201.load();
		} catch (Exception e) {
			LOGGER.error("Failed to load Beilin Entry Portability config", e);
			return;
		}
		BlockChangeRecorder1201.register(
			() -> addonConfig != null && addonConfig.recordingEnabled ? indexStore : null,
			LOGGER
		);
		if (!addonConfig.exportProcessingEnabled) {
			LOGGER.info("Beilin Entry Portability is installed. Recording={}, export processing disabled. Edit {} to enable WebSocket export processing.", addonConfig.recordingEnabled, PortabilityAddonConfig1201.configPath());
		}
		if (addonConfig.exportProcessingEnabled && !controlConfig.isApiKeyConfigured()) {
			LOGGER.warn("Beilin Entry Portability cannot start because {} has no valid apiKey.", SharedControlConfig1201.configPath());
		}
		...
		try {
			...
			if (addonConfig.exportProcessingEnabled && controlConfig.isApiKeyConfigured()) {
				...
				runtime = new PortabilityRuntime(
					...
					indexStore,
					new FabricWorldBlockReader1201(server, addonConfig.scanChunksPerTick),
