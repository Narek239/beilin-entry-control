...
public final class LoginHandler121x {
	...
	private static boolean forceDisconnect(Object handler, String message) {
		...
		BeilinEntryControl121x.LOGGER.error("forceDisconnect: no working disconnect for {}", handler.getClass().getName());
		...
		while (c != null) {
			for (Field f : c.getDeclaredFields()) {
				...
				try {
					...
					BeilinEntryControl121x.LOGGER.debug("Profile field get failed", e);
				}
			}
			c = c.getSuperclass();
		}
