...
abstract class WorldEditClipboardCommandsMixin {
	...
	private void beilinEntryPortability$wrapWorldEditCut(
		...
		ActorContext.Scope scope = BlockChangeRecorder1192.beginWorldEditCut(actor, editSession, region, leavePattern, mask);
	...
	private void beilinEntryPortability$wrapWorldEditPaste(
		...
		ActorContext.Scope scope = BlockChangeRecorder1192.beginWorldEditPaste(actor, editSession, session, atOrigin, onlySelect);
		boolean completed = false;
		try {
			original.call(
				actor,
